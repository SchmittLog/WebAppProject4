...
import { ZonedDateTime, ZoneId, DateTimeFormatter, Clock } from "@js-joda/core";
...
export default class WorldClock extends Vue {
  ...
  mounted(): void {
    ...
    this.timer = setInterval(() => {
      this.zdt = ZonedDateTime.now(ZoneId.of(this.timeZone));
      if (this.zdt.isBefore(ZonedDateTime.of(this.zdt.year(),this.zdt.monthValue(),this.zdt.dayOfMonth(),12,0,0,0,this.zdt.zone()))) {
        this.AmPm = "Am"
      } else {
        this.AmPm = "Pm"
      }
...
.Am {
  background-color: rgb(223, 99, 41);
}
